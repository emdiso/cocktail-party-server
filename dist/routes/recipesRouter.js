...
recipesRouter.delete("/delete_custom_recipe", authUtils_1.verifyToken, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const cr_id = req.query.crId;
    //TODO: Delete all connected information as well (image)
    return psqlConnection_1.default.query(`DELETE FROM custom_recipes cr WHERE cr.id = ${cr_id} AND cr.user_id = ${req.userId}`).then((result) => {
        return res.send("OK");
    }).catch(() => {
        return res.status(400).send();
    });
